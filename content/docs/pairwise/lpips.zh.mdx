...
- **相对度量准则**：LPIPS 本质上是深层相对感知距离，而非具有普适及格线的绝对标尺。人类视觉感知敏感度在不同任务间差异巨大，评估时必须在相同任务类型与图像分辨率下与 Baseline 基线相对比对。在风格迁移或图像编辑等涉及合法纹理重构与色彩变化的场景下，LPIPS 数值自然升高，并不代表结构破坏或语义撕裂。
