...
| 评测类别 | 指标名称 | 必填参数选项 | 核心机制与特征空间 | 取值方向 | 对应文档 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **单图无参考质量** | `aesthetic` | `--image` | OpenCLIP `ViT-L-14` + 线性回归打分头 | 数值越高越优 | [Aesthetic (单图美学)](/zh/no-reference/aesthetic) |
| **跨模态图文对齐** | `clip` | `--image`, `--prompt` | CLIP `clip-vit-base-patch32` 原始余弦相似度 | 数值越高越优 | [CLIP Score (图文对齐)](/zh/text-image/clip) |
| **跨模态图文对齐** | `pickscore` | `--image`, `--prompt` | CLIP ViT-H-14 大规模人类真实偏好对齐打分 | 数值越高越优 | [PickScore (偏好对齐)](/zh/text-image/pickscore) |
| **成对保真与主体** | `lpips` | `--image`, `--reference` | AlexNet 多尺度深层激活特征感知距离 | 数值越低越优 | [LPIPS (深度感知)](/zh/pairwise/lpips) |
| **成对保真与主体** | `ssim` | `--image`, `--reference` | $11 \times 11$ 高斯局部加权结构相似度 | 数值越高越优 | [SSIM (结构相似)](/zh/pairwise/ssim) |
| **成对保真与主体** | `psnr` | `--image`, `--reference` | 逐像素均方误差对数信噪比 (dB) | 数值越高越优 | [PSNR (峰值信噪比)](/zh/pairwise/psnr) |
| **成对保真与主体** | `arcface` | `--image`, `--reference` | InsightFace `buffalo_l` 512 维特征余弦距离 | 数值越低越优 | [ArcFace (主体保真)](/zh/pairwise/arcface) |
| **群体生成分布差异** | `fid` | `--image <dir>`, `--reference <dir>` | Inception-v3 pool3 特征二阶高斯统计距离 | 数值越低越优 | [FID (弗雷歇距离)](/zh/distribution/fid) |
| **群体生成分布差异** | `kid` | `--image <dir>`, `--reference <dir>` | Inception-v3 特征多项式核无偏 U 统计量 | 数值越低越优 | [KID (核最大均值)](/zh/distribution/kid) |

<EvaluationWorkflow lang="zh" />
